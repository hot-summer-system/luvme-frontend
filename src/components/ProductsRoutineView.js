...
export default function ProductsRoutineView({ products }) {
    ...
    const handleAddToRoutine = (item) => {
        const productExists = routineState.routingProductRequests.some(
            (product) => product.productId === item.productId
        );

        if (productExists) {
            alert('The product is already in your routine product list');
        } else {
            navigation.navigate('Modify Routine', { productToAdd: item });
        }
    };
    if (!fontsLoaded) {
        return null
        ...
        <FlatList
            ...
            renderItem={({ item }) => (
                <View style={styles.column}>
                    <TouchableOpacity onPress={() => handleAddToRoutine(item)}>
